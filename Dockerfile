...
# Environment variables for build (use dummy values to avoid build-time DB access)
ENV NODE_ENV=production
ENV NEXT_TELEMETRY_DISABLED=1
ENV DATABASE_URL=postgresql://dummy:dummy@localhost:5432/dummy
ENV BETTER_AUTH_SECRET=dummy-secret-for-build-time
ENV BETTER_AUTH_URL=http://localhost:3000
ENV GOOGLE_CLIENT_ID=dummy
ENV GOOGLE_CLIENT_SECRET=dummy
ENV RESEND_API_KEY=dummy
ENV EMAIL_SENDER_NAME=dummy
ENV EMAIL_SENDER_ADDRESS=dummy@dummy.com
